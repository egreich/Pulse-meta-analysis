...
# 3/30/2023: This is the "selection" model that picks between functions, 
...
model{
  for(i in 1:Nobs){ # number of observations
    ...
    # Mean model picks between two potential functions to describe the 
    ...
    # Uses an indicator, S, that picks off (selects) one of the functions. '
    # E.g., if S = 0, then pick Function2; if S = 1, then pick Function1:
  ...
  for(s in 1:Nstudy){ # number of studies
    ...
    # For selection model probability:
    w[s] ~ dunif(0,1)
  }
