...
model{
  ...
  for(p in 1:Npulse) {
    ...
    w[p] <- ifelse(w1[p]!=1 && w1[p]!=0, wx[p], w1[p]) # if the weight is not 0 or 1 (deterministic), use beta distribution (stochastic)
    #w1[p] ~ dunif(.5, .6) # the model doesn't actually use this, we code this in to avoid JAGS throwing an error, but we actually use wx when w1 is NA
    wx[p] ~ dbeta(a.w[sID[p]], b.w[sID[p]]) # beta distribution is for missing values only
  ...
  for(s in 1:Nstudy){ # number of studies
    ...
    #mu.w[s] <- mean(w[pulse_startID:pulse_stopID])
  }
